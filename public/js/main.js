$(document).ready(function () {
  ...
  function loadNotes() {
    ...
    $.get("/api/notes")
              ...
              <div class="card shadow-sm border-0 mb-4 animate__animated animate__fadeInUp">
                <div class="card-body">
                  ...
                  <blockquote class="blockquote ps-3 border-start border-primary">
                    <p class="mb-1 text-dark" style="font-size: 1.1rem;"><em>${escapeHtml(firstLine)}</em></p>
                  </blockquote>
                  <hr>
                  <p class="text-secondary fst-italic">🕊️ ${escapeHtml(rest)}</p>
                </div>
              </div>
            </section>
          `;
      ...
      .fail(() => {
        $("#supportNotes").html(`<div class="alert alert-danger">⚠️ Erreur de chargement des notes.</div>`);
      })
      .always(() => {
